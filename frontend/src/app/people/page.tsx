...
const RELATION_TYPES = ["FAMILY", "FRIEND", "COLLEAGUE", "MENTOR", "OTHER"];
...
export default function People() {
  ...
  const EDITABLE_FIELDS = [
    "name", "nickname", "company", "occupation", "bio", "email", "phone",
    "birthday", "anniversary", "city", "country", "meeting_place",
    "linkedin_url", "twitter_handle", "instagram_handle", "github_username",
    "profile_notes", "topics_of_interest", "hobbies", "contact_frequency_days",
  ...
  const save = async () => {
    ...
    try {
      const norm = (value: any) => Array.isArray(value) ? value.join(", ") : String(value ?? "").trim();
      for (const field of EDITABLE_FIELDS) {
        const value = norm(form[field]);
        if (value === norm(selected[field])) continue;
  ...
  const removePerson = async () => {
    if (!selected) return;
    if (!window.confirm(`Delete ${selected.name}? This archives the contact and removes them from the board and graph.`)) return;
    setSaving(true); setError("");
    try {
      await apiWrite(`/relationship/person/${selected.id}`, "DELETE", {});
      setSelected(null); setDetail(null); setEditing(false);
      await load();
    } catch (e: any) { setError(e.message); } finally { setSaving(false); }
  };
  ...
  const movePerson = async (personId: string, target: string) => {
    if (!personId) return;
    setSaving(true); setError("");
    try { await apiWrite(`/relationship/person/${personId}`, "PATCH", { field: "category", value: target }); await load(); }
    catch (e: any) { setError(e.message); } finally { setSaving(false); }
  };
...
function EditForm({ form, setForm, onSave, saving }: { form: Person; setForm: (f: Person) => void; onSave: () => void; saving: boolean }) {
  const set = (key: string) => (e: any) => setForm({ ...form, [key]: e.target.value });
  const list = (key: string) => Array.isArray(form[key]) ? (form[key] as string[]).join(", ") : form[key] ?? "";
  const date = (key: string) => { const v = form[key]; return typeof v === "string" && v.length >= 10 ? v.slice(0, 10) : (v ?? ""); };
  return <div className="people-form edit-form">
    {[["name", "Name"], ["nickname", "Nickname"], ["occupation", "Position"], ["company", "Company"]].map(([key, label]) => <label key={key}><span>{label}</span><input value={form[key] ?? ""} onChange={set(key)} /></label>)}
    <label><span>About</span><textarea value={form.bio ?? ""} onChange={set("bio")} /></label>
    {[["email", "Email"], ["phone", "Phone"], ["birthday", "Birthday"], ["anniversary", "Anniversary"], ["city", "City"], ["country", "Country"], ["meeting_place", "Where we met"]].map(([key, label]) => <label key={key}><span>{label}</span><input type={["birthday", "anniversary"].includes(key) ? "date" : "text"} value={date(key)} onChange={set(key)} /></label>)}
    <label><span>Category</span><select value={form.category ?? "NETWORK"} onChange={set("category")}>{CATEGORIES.map((c) => <option key={c}>{c}</option>)}</select></label>
    <label><span>Relation type</span><select value={form.relation_type ?? "OTHER"} onChange={set("relation_type")}>{RELATION_TYPES.map((r) => <option key={r}>{r}</option>)}</select></label>
    <label><span>Contact every (days)</span><input type="number" min="1" value={form.contact_frequency_days ?? ""} onChange={set("contact_frequency_days")} /></label>
    {[["linkedin_url", "LinkedIn URL"], ["twitter_handle", "Twitter"], ["instagram_handle", "Instagram"], ["github_username", "GitHub"]].map(([key, label]) => <label key={key}><span>{label}</span><input value={form[key] ?? ""} onChange={set(key)} /></label>)}
    <label><span>Profile notes</span><textarea value={form.profile_notes ?? ""} onChange={set("profile_notes")} /></label>
    <label><span>Topics, comma separated</span><input value={list("topics_of_interest")} onChange={set("topics_of_interest")} /></label>
    <label><span>Hobbies, comma separated</span><input value={list("hobbies")} onChange={set("hobbies")} /></label>
    <button className="people-add" onClick={onSave} disabled={saving}>{saving ? "Saving…" : "Save card"}</button>
  </div>; }
